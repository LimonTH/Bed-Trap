...
import static me.bedtrap.addon.util.advanced.DeathUtils.getTargets;
import static me.bedtrap.addon.util.basic.EntityInfo.getName;
import static me.bedtrap.addon.util.basic.EntityInfo.isDead;
...
public class AutoEz extends Module {
    ...
    public void onKill(PlayerEntity player) {
        if (isActive()) {
            kills++;
            TextUtils.sendPlayerMsg(Config.get().prefix + "say " + getMessage(player, MessageType.Kill));
        }
    }
    ...
    private void onPop(PacketEvent.Receive event) {
        ...
        synchronized (totemPopMap) {
            ...
            if (skips >= skipMessage.get() && getTargets().contains(getName((PlayerEntity) entity))) {
                TextUtils.sendPlayerMsg(Config.get().prefix + "say " + getMessage((PlayerEntity) entity, MessageType.Pop));
                skips = 0;
            } else skips++;
