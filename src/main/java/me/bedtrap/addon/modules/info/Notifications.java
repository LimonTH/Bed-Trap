...
import static me.bedtrap.addon.modules.hud.ToastNotifications.addToast;
...
public class Notifications extends Module {
    ...
    public Notifications() {
        super(BedTrap.Info, "Notifications", "Sends messages in hud about different events.");
    }
    ...
    private void onReceivePacket(PacketEvent.Receive event) {
        ...
        synchronized (totemPopMap) {
            ...
            if (totemNotif.get()) send(entity.getName().getString() + " popped " + pops + " time`s!", notifications);
        }
    }
        ...
        synchronized (totemPopMap) {
            for (PlayerEntity player : mc.world.getPlayers()) {
                ...
                if (player.deathTime > 0 || player.getHealth() <= 0) {
                    ...
                    if (deaths.get())
                        send(player.getName().getString() + " just died after " + pops + " pops!", notifications);
    ...
    private void notifySurroundBreak(PlayerEntity player) {
        send("Your surround is being broken by " + player.getName().getString(), notifications);
    }
