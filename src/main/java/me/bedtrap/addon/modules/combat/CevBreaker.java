...
import static me.bedtrap.addon.util.basic.BlockInfo.getState;
...
public class CevBreaker extends Module {
    ...
    public CevBreaker() {
        super(BedTrap.Combat, "CEV-breaker", "Automatically breaks target's CEV.");
    }
    ...
    public void onTick(TickEvent.Post event) {
        ...
        if (((ClientPlayerInteractionManagerAccessor) mc.interactionManager).getBreakingProgress() <= 0.01)
            breakPos = validPos(target);
    ...
    private BlockPos validPos(PlayerEntity player) {
        ...
        if (getState(pos.up(3)).isAir() && (getState(pos.up(2)).isOf(Blocks.OBSIDIAN) || getState(pos.up(2)).isOf(Blocks.AIR)))
            return pos.up(2);
        List<BlockPos> posList = new ArrayList<>();

        for (CardinalDirection direction : CardinalDirection.values()) {
            if (getState(pos.offset(direction.toDirection()).up(2)).isAir() && (getState(pos.offset(direction.toDirection()).up(1)).isOf(Blocks.OBSIDIAN) || getState(pos.offset(direction.toDirection()).up(1)).isOf(Blocks.AIR)))
                posList.add(pos.offset(direction.toDirection()).up(1));
        }

        posList.sort(Comparator.comparingDouble(PlayerUtils::distanceTo));
        return posList.isEmpty() ? null : posList.getFirst();
    }
