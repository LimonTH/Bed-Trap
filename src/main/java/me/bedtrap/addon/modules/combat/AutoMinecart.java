...
public class AutoMinecart extends Module {
    ...
    public AutoMinecart() {
        super(BedTrap.Combat, "auto-minecart", "Places and blow's up minecarts in the target.");
    }
    ...
    public void onTick(TickEvent.Post event) {
        ...
        if (ticks == delay.get() - 1)
            BlockUtils.place(currentPos, rails, rotate.get(), 50, swing.get(), false, swapBack.get());
        if (ticks == delay.get() - 2)
            Interaction.interactBlock(currentPos, tntMinecart.slot(), rotate.get(), swapBack.get(), swing.get(), false);
        if (ticks == delay.get() - 3)
            Interaction.breakBlock(currentPos, pickaxe.slot(), rotate.get(), false, swing.get(), false);
        if (ticks == delay.get() - 5)
            Interaction.interactBlock(currentPos.down(), ignite.slot(), rotate.get(), swapBack.get(), swing.get(), false);
    ...
    public void onRender(Render3DEvent event) {
        ...
        event.renderer.box(x, y, z, x + 1D, y + 0.15, z + 1D, railSide.get(), railLine.get(), shapeMode.get(), 0); // rail render
        event.renderer.box(x, y + 1D, z, x + 1D, y + 1 - 0.85, z + 1D, cartSide.get(), cartLine.get(), shapeMode.get(), 0); // cart render
    }
    ...
    private void onRender2D(final Render2DEvent event) {
        ...
        Vector3d pos = new Vector3d(placePos.getX() + 0.5, placePos.getY() + 0.5, placePos.getZ() + 0.5);
    ...
    public static class Interaction {
        ...
        public static boolean breakBlock(BlockPos pos, int slot, boolean rotate, boolean swapBack, boolean swing, boolean debug) {
            ...
            if (rotate)
                Rotations.rotate(Rotations.getYaw(pos), Rotations.getPitch(pos), 50, () -> sendBreakPacket(pos));
