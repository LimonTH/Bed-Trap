...
import me.bedtrap.addon.modules.info.Notifications;
import me.bedtrap.addon.util.advanced.BedUtils;
import me.bedtrap.addon.util.basic.BlockInfo;
import me.bedtrap.addon.util.basic.Vec3dInfo;
...
import static me.bedtrap.addon.util.advanced.BedUtils.*;
...
public class BedBomb extends Module {
    ...
    public BedBomb() {
        super(BedTrap.Combat, "bed-bomb", "Automatically places and interact beds in Nether.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (pauseOnCA.get() && (Modules.get().get(meteordevelopment.meteorclient.systems.modules.combat.AutoCrystal.class).isActive() || Modules.get().get(AutoCrystal.class).isActive()))
            return;
    ...
    public void onBreakPacket(PacketEvent.Receive event) {
        ...
        if (event.packet instanceof BlockBreakingProgressS2CPacket packet) {
            ...
            if (Boolean && packetBp.equals(finalPos.offset(placeDirection.toDirection())))
                smartLay = false;
    ...
    private void doPlace() {
        ...
        Rotations.rotate(y, p, 1000000, () -> {
            ...
            mc.getNetworkHandler().sendPacket(new PlayerInteractBlockC2SPacket(Hand.MAIN_HAND, placeResult, mc.world.getPendingUpdateManager().incrementSequence().getSequence()));
    ...
    private void calculatePos() {
        ...
        try {
            for (BlockPos p : sphere) {
                ...
                if (!Vec3dInfo.isWithinRange(BlockInfo.closestVec3d(p), placeRange.get()) || !isReplaceable(p))
                    continue;
    ...
    private boolean intersectsWithEntities(BlockPos blockPos) {
        Box box = new Box(
                blockPos.getX(), blockPos.getY(), blockPos.getZ(), blockPos.getX() + 1D, blockPos.getY() + 0.6, blockPos.getZ() + 1D
        );
