...
import static me.bedtrap.addon.util.basic.EntityInfo.getName;
...
public class AnchorBomb extends Module {
    ...
    private void doPlace(BlockPos blockPos, boolean place) {
        ...
        if (placeTimer == 2) {
            ...
            mc.getNetworkHandler().sendPacket(new PlayerInteractBlockC2SPacket(anchor.getHand(), result, mc.world.getPendingUpdateManager().incrementSequence().getSequence()));
            Interaction.doSwing(anchor.getHand(), false);
        }
        if (placeTimer == 1) {
            Interaction.updateSlot(glowstone, true);
            mc.getNetworkHandler().sendPacket(new PlayerInteractBlockC2SPacket(glowstone.getHand(), result, mc.world.getPendingUpdateManager().incrementSequence().getSequence()));
