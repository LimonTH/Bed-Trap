...
import me.bedtrap.addon.util.advanced.CrystalUtils;
import me.bedtrap.addon.util.advanced.Interaction;
import me.bedtrap.addon.util.advanced.RenderUtils;
import me.bedtrap.addon.util.basic.RenderInfo;
...
import static me.bedtrap.addon.util.advanced.RenderUtils.*;
import static me.bedtrap.addon.util.basic.BlockInfo.X;
import static me.bedtrap.addon.util.basic.BlockInfo.Y;
import static me.bedtrap.addon.util.basic.BlockInfo.Z;
...
public class AutoCrystal extends Module {
    ...
    private void onPacketReceive(PacketEvent.Receive event) {
        if (event.packet instanceof EntityVelocityUpdateS2CPacket packet
                && bestTarget != null
                && packet.getEntityId() == bestTarget.getId()
    ...
    private void doBreak(Entity crystal) {
        ...
        if (antiWeakness.get()) {
            ...
            // Check for strength
            if (weakness != null && (strength == null || strength.getAmplifier() <= weakness.getAmplifier()) && !isValidWeaknessItem(mc.player.getMainHandStack())) {
    ...
    private void attackCrystal(Entity entity) {
        ...
        if (Modules.get().isActive(Criticals.class)) {
            Modules.get().get(Criticals.class).toggle();
            BedTrap.log("Idiot, disable Criticals on CA working! Disabled automatically.");
        }
    ...
    private void placeCrystal(BlockHitResult result, double damage, BlockPos supportBlock) {
        ...
        if (supportBlock == null) {
            // Place crystal
            mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(hand, result, mc.world.getPendingUpdateManager().incrementSequence().getSequence()));
    ...
    public boolean doYawSteps(double targetYaw, double targetPitch) {
        ...
        } else if (delta < 180) yaw -= yawSteps.get();
        else yaw += yawSteps.get();
    ...
    public boolean shouldFacePlace() {
        if (!facePlace.get()) return true;

        for (PlayerEntity target : targets) {
            if (!isSurrounded(target) || isTrapped(target)) return true;
            if (facePlaceHurt.get() && target.hurtTime != 0) return true;

            if (forceFacePlace.get().isPressed()) return false;
            if (EntityUtils.getTotalHealth(target) <= facePlaceHealth.get()) return false;
            ...
            for (ItemStack itemStack : target.getArmorItems()) {
                if (facePlaceArmor.get()) {
                    if (itemStack == null || itemStack.isEmpty()) return false;
                        ...
                        if (shouldBreakArmor) return false;
                    }
                }
            }
        }
    ...
    public class RenderBlock {
        ...
        public void render(RenderInfo ri, Color sides, Color lines) {
            ...
            sides.a = (int) (sides.a * ((double) this.ticks / (double) fadeAmount.get()));
            lines.a = (int) (lines.a * ((double) this.ticks / (double) fadeAmount.get()));
