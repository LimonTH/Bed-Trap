...
public class Surround extends Module {
    private static final List<BlockPos> posPlaceBlocks = new ArrayList<>();
    ...
    public Surround() {
        super(BedTrap.Combat, "Surround+", "Surrounds you in blocks to prevent crystal damage");
    }
        ...
        for (Direction side : Direction.values()) {
            ...
            if (forceSurround.get()) {
                if (packet.get()) packetPlace(blockPos.offset(side));
                    ...
                    BlockUtils.place(blockPos.offset(side), InvUtils.findInHotbar(itemStack -> blocks.get().contains(Block.getBlockFromItem(itemStack.getItem()))), rotate.get(), 20, swing.get(), false);
        ...
        for (BlockPos p : posPlaceBlocks) {
            if (BlockUtils.canPlace(p, false) && ticks <= bpt.get()) {
                ...
                if (packet.get()) packetPlace(p);
                    ...
                    BlockUtils.place(p, InvUtils.findInHotbar(itemStack -> blocks.get().contains(Block.getBlockFromItem(itemStack.getItem()))), rotate.get(), 20, swing.get(), false);
                if (AGB.get())
                    mc.player.networkHandler.sendPacket(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.STOP_DESTROY_BLOCK, p, Direction.UP));
    ...
    private void packetPlace(BlockPos pos) {
        if (pos != null) {
            ...
            mc.getNetworkHandler().sendPacket(new PlayerInteractBlockC2SPacket(Hand.MAIN_HAND, result, mc.world.getPendingUpdateManager().incrementSequence().getSequence()));
    ...
    private boolean inCenter() {
        ...
        int count = 0;
        if (mc.player.getBlockPos().equals(new BlockPos((int) (mc.player.getX() - (mc.player.getWidth() + 0.1) / 2.0), (int) mc.player.getY(), (int) (mc.player.getZ() - (mc.player.getWidth() + 0.1) / 2.0)))) {
            count++;
        }
        if (mc.player.getBlockPos().equals(new BlockPos((int) (mc.player.getX() + (mc.player.getWidth() + 0.1) / 2.0), (int) mc.player.getY(), (int) (mc.player.getZ() + (mc.player.getWidth() + 0.1) / 2.0)))) {
            count++;
    ...
    private class RenderBlock {
        ...
        public void render(Render3DEvent event, Color sides, Color lines, Color sides2, Color lines2, ShapeMode shapeMode, double lineSize) {
            ...
            sides.a *= (int) ((double) ticks / 8);
            lines.a *= (int) ((double) ticks / 8);
            sides2.a *= (int) ((double) ticks / 8);
            lines2.a *= (int) ((double) ticks / 8);
