...
public class AutoBuildRewrite extends Module {
    ...
    public AutoBuildRewrite() {
        super(BedTrap.Misc, "auto-build-rewrite", "Places build that you choose.");
    }
    ...
    public void onTick(TickEvent.Post event) {
        ...
        for (BlockPos blockPo : blockPos) {
            if (BlockUtils.canPlace(blockPo, true) && ticks <= bpt.get()) {
                BlockUtils.place(blockPo, findBlock(), 50);
                ticks++;
            }
        }
    }
    ...
    public BlockPos add(int x, int y, int z) {
        return new BlockPos((int) (mc.player.getX() + x), (int) (mc.player.getY() + y), (int) (mc.player.getZ() + z));
    }
