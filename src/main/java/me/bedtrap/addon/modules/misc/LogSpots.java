...
public class LogSpots extends Module {
        ...
        if (event.entity instanceof PlayerEntity) {
            ...
            for (int i = 0; i < players.size(); i++) {
                if (players.get(i).uuid.equals(event.entity.getUuid())) {
                    if (notification.get())
                        ToastNotifications.addToast(players.get(i).name + " Just logged back!", new Color(100, 100, 255));
                    toRemove = i;
                    break;
                }
            }
    ...
    private class Entry {
        ...
        public Entry(PlayerEntity entity) {
            if (notification.get()) ToastNotifications.addToast(entity.getName().getString() + " Just logged out!");
