...
public class PacketUtils {
    ...
    private static boolean hasAquaAffinity(PlayerEntity player) {
        for (ItemStack armorPart : player.getArmorItems()) {
            if (Utils.hasEnchantment(armorPart, Enchantments.AQUA_AFFINITY)) {
                return true;
            }
        }
        return false;
    ...
    private double getBlockBreakingSpeed(int slot, BlockState block) {
        ...
        if (mc.player.isSubmergedIn(FluidTags.WATER) && !hasAquaAffinity(mc.player)) {
            speed /= 5.0F;
        }
