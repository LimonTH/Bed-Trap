...
import static me.bedtrap.addon.util.basic.EntityInfo.getBoundingBox;
import static me.bedtrap.addon.util.basic.EntityInfo.getVelocity;
...
public class PredictionUtils {
    ...
    public static Box returnPredictBox(PlayerEntity entity, boolean collision, int i) {
        ...
        if (collision) {
            Vec3i vec3i = new Vec3i((int) pBox.getCenter().x, (int) pBox.getCenter().y, (int) pBox.getCenter().z);
            ArrayList<BlockPos> l = new ArrayList<>(getPredictCollisionBlocks(new BlockPos(vec3i)));
            for (BlockPos p : l) {
                Box bBox = new Box(p);
